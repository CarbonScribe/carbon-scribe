...
describe('TrackingService', () => {
    ...
    describe('addStatusEvent', () => {
        it('should create a status event and update order status', async () => {
            const event = { id: 'evt-1', orderId: '1', status: 'processing', message: 'Processing started' };
            mockPrisma.orderStatusEvent.create.mockResolvedValue(event);
            mockPrisma.order.update.mockResolvedValue({});

            const result = await service.addStatusEvent('1', 'processing', 'Processing started');

            expect(result).toEqual(event);
            expect(mockPrisma.orderStatusEvent.create).toHaveBeenCalledWith({
                data: { orderId: '1', status: 'processing', message: 'Processing started' },
            });
    ...
    describe('getStatusHistory', () => {
        it('should return ordered status events', async () => {
            const events = [
                ...
                { id: 'evt-2', status: 'processing', createdAt: new Date('2024-01-02') },
            ];
