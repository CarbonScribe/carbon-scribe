...
export class QualityService {
  ...
  async getQualityBreakdown(id: string): Promise<CreditQualityBreakdown> {
    ...
    const dynamicScore = Math.round((verificationScore * 0.25) + (additionalityScore * 0.2) + (permanenceScore * 0.2) + ((100 - leakageScore) * 0.15) + (cobenefitsScore * 0.1) + (transparencyScore * 0.1));
